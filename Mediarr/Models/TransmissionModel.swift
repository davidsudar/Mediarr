...
public enum TorrentPriority: String, CaseIterable {
...
public struct Torrent: Codable, Hashable, Identifiable {
    public let id: Int
    var totalSize, status, peersConnected, eta, leftUntilDone, sizeWhenDone, downloadedEver, downloadSpeed, queuePosition: Int
    var sendingToUs: Int?
    let name: String
    var percentDone: Double
    
    enum CodingKeys: String, CodingKey {
        case id, totalSize, status, peersConnected, eta, leftUntilDone, sizeWhenDone, downloadedEver, downloadSpeed = "rateDownload", queuePosition
        case sendingToUs
        case name
        case percentDone
    }
...
public func getTorrents(config: TransmissionConfig, auth: TransmissionAuth, onReceived: @escaping ([Torrent]?, String?) -> Void) -> Void {
    ...
    let requestBody = TransmissionListRequest(
        method: "torrent-get",
        arguments: [
            "fields": [ "id", "name", "totalSize", "percentDone", "status", "sendingToUs", "peersConnected", "eta", "leftUntilDone", "sizeWhenDone", "downloadedEver", "rateDownload", "queuePosition" ]
        ]
    )
...
struct TransmissionSessionStatsArguments: Codable {
    let activeTorrentCount, downloadSpeed, pausedTorrentCount, torrentCount, uploadSpeed: Int
}

struct TransmissionSessionStatsResponse: Codable {
    let arguments: TransmissionSessionStatsArguments
}
...
/// - Parameter auth: The username and password for the server
/// - Parameter onResponse: An escaping function that receives the response from the server
func getSessionStats(config: TransmissionConfig, auth: TransmissionAuth, onResponse: @escaping (TransmissionSessionStatsArguments?) -> Void) {
    url = config
    url?.scheme = "http"
    url?.path = "/transmission/rpc"
    url?.port = config.port ?? 443
    ...
    let task = URLSession.shared.dataTask(with: req) { (data, resp, error) in
        ...
        case 409?: // If we get a 409, save the token and try again
            authorize(httpResp: httpResp)
            getSessionStats(config: config, auth: auth, onResponse: onResponse)
            return
        case 401?:
            return onResponse(nil)
        case 200?:
            let response = try? JSONDecoder().decode(TransmissionSessionStatsResponse.self, from: data!)
            return onResponse(response?.arguments)
        default:
            return onResponse(nil)
        }
    }
...
/// Set a transfers quesue position
///
/// - Parameter torrent: The torrent whose priority we are setting
/// - Parameter queuePosition: int 0..n
/// - Parameter onComplete: Called when the servers' response is received with a `TransmissionResponse`
public func increaseQueuePosition(torrent: Torrent, info: (config: TransmissionConfig, auth: TransmissionAuth), onComplete: @escaping (TransmissionResponse) -> Void) {
    url = info.config
    url?.scheme = "http"
    url?.path = "/transmission/rpc"
    url?.port = info.config.port ?? 443
    ...
    let task = URLSession.shared.dataTask(with: req) { (data, resp, err) in
        ...
        case 409?: // If we get a 409, save the token and try again
            authorize(httpResp: httpResp)
            increaseQueuePosition(torrent: torrent, info: info, onComplete: onComplete)
            return
        case 401?:
            return onComplete(TransmissionResponse.forbidden)
        case 200?:
...
func makeConfig(server: String, port: Int, username: String, password: String) -> (config: TransmissionConfig, auth: TransmissionAuth) {
...
func getEta(eta: Int) -> String{
    ...
    if days == 0 && hours > 0
    {
        returnString = String(format: "%0.2d Hours, %0.2d Minutes Remaining",hours,minutes)
        
    }
    else if hours == 0 && minutes > 0 {
        returnString = String(format: "%0.2d Minutes, %0.2d Seconds Remaining",minutes,seconds)
    }
    else if minutes == 0 && seconds > 0 {
        returnString = String(format: "%0.2d Seconds Remaining",seconds)
    }
    else {
        returnString = String(format: "%0.2d Days, %0.2d Hours Remaining",days,hours)
    }
    return returnString
}

struct ParsedShow: Hashable, Decodable {
    let title: String
    let parsedEpisodeInfo: ParsedEpisodeInfo
}
struct ParsedEpisodeInfo: Hashable, Decodable {
